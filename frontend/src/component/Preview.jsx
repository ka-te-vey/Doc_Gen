      ...
      <div className="panel-inner h-full min-h-0 flex flex-col gap-4">
        <div className="flex flex-col gap-4 rounded-[20px] px-6 py-5 text-[11px] uppercase tracking-[0.4em] font-black border-b border-white/5 relative z-10 md:flex-row md:items-center md:justify-between" style={{ backgroundColor: 'var(--preview-bg)' }}>
          <div className="flex items-center gap-4">
            ...
            <span className="font-bold" style={{ color: 'var(--text-h)' }}>PREVIEW</span>
          </div>
        ...
        <div className="flex-1 min-h-0 rounded-[20px] p-6 md:p-8 relative z-10" style={{ backgroundColor: 'var(--code-bg)' }}>
            {documentData.content ? (
              <div className="space-y-8 text-sm leading-relaxed h-full min-h-0 flex flex-col relative" >
                <div className="flex items-center justify-between text-[10px] uppercase tracking-[0.4em] font-bold border-b border-white/5 pb-4 mb-2" style={{ color: 'var(--text-h)' }}>
                  <span>{documentData.title || 'Untitled Document'}</span>
                  <span>{documentData.documentType || 'README'}</span>
                </div>
                <div className="flex-1 min-h-0 overflow-y-auto pr-4 custom-scrollbar relative group/preview">
                  <div className="prose prose-sm max-w-none dark:prose-invert 
                    prose-headings:font-black prose-headings:tracking-tight
                    prose-p:leading-relaxed
                    prose-code:text-blue-600 dark:prose-code:text-blue-300 prose-code:bg-blue-500/10 dark:prose-code:bg-blue-900/20 prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded-sm
              ...
              <div className="h-full flex items-center justify-center relative">
                <div className="text-center">
                  <div className="text-3xl font-black mb-6 tracking-tighter" style={{ color: 'var(--text-h)'}}>NO DOCUMENTATION YET</div>
                  <div className="text-[10px] uppercase tracking-[0.5em] font-bold" style={{ color: 'var(--text-h)'}}>PASTE YOUR CODE AND HIT GENERATE</div>
                </div>
              </div>
            )}
        </div>
