...
export default function ExportModal({ documentData, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-3 bg-blur">
      <div id="export-modal" className="w-full max-w-3xl rounded-2xl border shadow-2xl overflow-hidden" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        <div className="border-b p-3" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--code-bg)' }}>
          <div className="flex items-center justify-between">
            ...
            <button onClick={onClose} className="rounded-md border border-white/20 px-2 py-1 text-xs hover:bg-white/10 transition">✕</button>
          </div>
        </div>

        <div className="p-4 space-y-4">
          <div ref={previewRef} className="rounded-xl border p-4 max-h-[60vh] overflow-y-auto custom-scrollbar" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--code-bg)', color: 'var(--text)' }}>
                    ...
                    code({node, inline, className, children, ...props}) {
                      ...
                      return !inline && match ? (
                        <SyntaxHighlighter
                          ...
                          language={match[1]}
                          PreTag="div"
                          className="rounded-lg !my-4 !text-[11px]"
                          {...props}
                        >
                          {String(children).replace(/\n$/, '')}
                        </SyntaxHighlighter>
                      ) : (
                        <code className={`${className} bg-black/5 dark:bg-white/5 rounded px-1.5 py-0.5 font-mono text-[11px]`} {...props}>
                          {children}
                        </code>
                      )
                    },
                    table({children}) {
                      return <div className="overflow-x-auto my-4 rounded-lg border border-black/10 dark:border-white/10"><table className="min-w-full divide-y divide-black/10 dark:divide-white/10">{children}</table></div>
                    },
                    th({children}) {
